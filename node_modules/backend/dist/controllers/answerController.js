...
export async function submitAnswers(req, res) {
    ...
    for (const a of parsed.data.answers) {
        ...
        if (!q)
            return badRequest(res, `Unknown questionId: ${a.questionId}`);
        if (q.type === 'text') {
            const schema = z.string().min(1).max(2000);
            const okText = schema.safeParse(a.value);
            if (!okText.success)
                return badRequest(res, `Invalid text answer for ${q.id}`, okText.error.flatten());
        }
        else if (q.type === 'single') {
            const choices = z
                .object({ choices: z.array(z.string()) })
                .safeParse(q.options ?? {})
                .success
                ? q.options.choices
                : [];
            ...
            if (!okSingle.success)
                return badRequest(res, `Invalid single answer for ${q.id}`);
            ...
            if (!okMulti.success)
                return badRequest(res, `Invalid multiple answer for ${q.id}`);
            if (choices.length && okMulti.data.some((x) => !choices.includes(x)))
                return badRequest(res, `Answer contains invalid choice for ${q.id}`);
        }
    }
    await prisma.$transaction(parsed.data.answers.map((a) => prisma.answer.create({
        data: {
...
export async function adminListAnswers(req, res) {
